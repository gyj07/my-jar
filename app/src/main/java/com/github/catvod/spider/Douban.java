...
public class Douban extends Spider {
    ...
    // ============================================================
    // ★ parse_item（改：图片拼 Referer/UA + vod_id 用 id）
    // ============================================================
    private JSONObject parseItem(JSONObject item) throws Exception {
        ...
        if (pic != null) {
            picUrl = pic.optString("normal", "");
            if (TextUtils.isEmpty(picUrl)) picUrl = pic.optString("large", "");
        }
        ...
        // ★ 过滤没图片的
        if (TextUtils.isEmpty(picUrl)) return null;

        JSONObject vod = new JSONObject();
        // ★ vod_id 用豆瓣 item.id
        vod.put("vod_id", "msearch:" + item.optString("id", ""));
        vod.put("vod_name", title);
        vod.put("vod_pic", picUrl);
        vod.put("vod_remarks", TextUtils.isEmpty(score) ? "" : "评分：" + score);
        return vod;
    }
